...
const polishes = ['Excellent', 'Very Good', 'Good'] as const;
const fluorescences = ['None', 'Faint', 'Medium', 'Strong'] as const;
...
async function main() {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    diamondData.push({
      ...
      certificateLab: lab as 'GIA' | 'IGI' | 'AGS',
      ...
      isLabGrown,
      originCountry: isLabGrown
        ? null
        : randomFrom(['India', 'Russia', 'Botswana', 'South Africa', 'Canada']),
  ...
  for (const diamond of diamondData) {
    ...
    const created = await prisma.diamond.upsert({
      ...
      update: {},
      create: diamondFields as Parameters<typeof prisma.diamond.create>[0]['data'],
    });
  ...
  console.log('Indexing diamonds into Elasticsearch...');
  try {
    const allDiamonds = await prisma.diamond.findMany({
      include: { images: { take: 1, orderBy: { sortOrder: 'asc' } } },
    });

    // Ensure index exists
    const indexExists = await esClient.indices.exists({ index: 'diamonds' });
    if (!indexExists) {
      await esClient.indices.create({ index: 'diamonds' });
    }

    if (allDiamonds.length > 0) {
      const body = allDiamonds.flatMap((d) => [
        { index: { _index: 'diamonds', _id: d.id } },
        { ...d, priceInr: Number(d.priceInr) },
      ]);
      await esClient.bulk({ body });
      await esClient.indices.refresh({ index: 'diamonds' });
    }

    console.log(`✅ ${allDiamonds.length} diamonds indexed into Elasticsearch`);
  } catch (err) {
    console.warn(`⚠️  Elasticsearch indexing skipped (not critical): ${(err as Error).message}`);
  }
  ...
  const ringSettingsData = [
    ...
    // Halo settings
    {
      ...
      sideStones: JSON.stringify([
        { shape: 'ROUND', totalCarat: 0.25, color: 'F', clarity: 'VS1', count: 30 },
      ]),
      engravingAvailable: true,
      maxEngravingChars: 20,
      basePrice: 65000,
    ...
    },
    {
      ...
      centerProng: 4,
      sideStones: JSON.stringify([
        { shape: 'ROUND', totalCarat: 0.2, color: 'G', clarity: 'VS2', count: 24 },
      ]),
      engravingAvailable: true,
      maxEngravingChars: 20,
      basePrice: 78000,
    ...
    },
    {
      ...
      centerProng: 4,
      sideStones: JSON.stringify([
        { shape: 'ROUND', totalCarat: 0.4, color: 'F', clarity: 'VVS2', count: 50 },
      ]),
      engravingAvailable: true,
      maxEngravingChars: 20,
      basePrice: 98000,
    ...
    // Three-stone settings
    {
      ...
      centerProng: 4,
      sideStones: JSON.stringify([
        { shape: 'OVAL', totalCarat: 0.5, color: 'G', clarity: 'VS2', count: 2 },
      ]),
      engravingAvailable: true,
      maxEngravingChars: 20,
      basePrice: 85000,
    ...
    },
    {
      ...
      centerProng: 4,
      sideStones: JSON.stringify([
        { shape: 'ROUND', totalCarat: 0.4, color: 'H', clarity: 'VS1', count: 2 },
      ]),
      engravingAvailable: true,
      maxEngravingChars: 20,
      basePrice: 75000,
    ...
    // Vintage settings
    {
      ...
      centerProng: 4,
      sideStones: JSON.stringify([
        { shape: 'ROUND', totalCarat: 0.15, color: 'G', clarity: 'VS2', count: 20 },
      ]),
      engravingAvailable: true,
      maxEngravingChars: 20,
      basePrice: 72000,
    ...
    },
    {
      ...
      centerProng: 4,
      sideStones: JSON.stringify([
        { shape: 'baguette', totalCarat: 0.3, color: 'F', clarity: 'VS1', count: 4 },
      ]),
      engravingAvailable: true,
      maxEngravingChars: 20,
      basePrice: 92000,
    ...
    // Pavé setting
    {
      ...
      centerProng: 4,
      sideStones: JSON.stringify([
        { shape: 'ROUND', totalCarat: 0.5, color: 'F', clarity: 'VS2', count: 60 },
      ]),
      engravingAvailable: true,
      maxEngravingChars: 15,
      basePrice: 58000,
  ...
  for (const setting of ringSettingsData) {
    ...
    const created = await prisma.ringSetting.upsert({
      ...
      update: {},
      create: settingFields as Parameters<typeof prisma.ringSetting.create>[0]['data'],
    });
